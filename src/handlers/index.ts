...
export async function handleSolutionAsync(slnPath: string) {
	...
	vscode.window.showInformationMessage(
		"Usings moved to GlobalUsings.cs for all projects in solution!",
	);
}

async function getCsprojPathsFromSln(
	slnPath: string,
	slnDir: string,
): Promise<string[]> {
	...
	if (usings.length > 0) {
		...
		if (await fileExists(globalUsingsPath)) {
			...
			existingContent
				...
				.forEach((l) => {
					existingGlobalUsings.add(l);
				});
		}
		usings.forEach((u) => {
			existingGlobalUsings.add(`global ${u}`);
		});
...
export async function handleSlnxFileAsync(slnxPath: string) {
	...
	await Promise.all(csprojPaths.map(handleProjectAsync));
	vscode.window.showInformationMessage(
		"Usings moved to GlobalUsings.cs for all projects in .slnx solution!",
	);
}
...
async function findCsFilesRecursively(dir: string): Promise<string[]> {
	...
	for (const dirent of dirents) {
		const name = dirent.name; // Dirent object
		...
		if (
			...
			if (skippedDirs.includes(name) || name.startsWith(".")) {
				continue;
			}
...
export function extractUsings(content: string): {
	usings: string[];
...
function formatGlobalUsings(usingsSet: Set<string>): string {
	const sortedUsings = Array.from(usingsSet).sort((a, b) => {
		...
		if (nsA.startsWith("System") && !nsB.startsWith("System")) {
			return -1;
		}
		if (!nsA.startsWith("System") && nsB.startsWith("System")) {
			return 1;
		}
		if (nsA.startsWith("Microsoft") && !nsB.startsWith("Microsoft")) {
			return -1;
		}
		if (!nsA.startsWith("Microsoft") && nsB.startsWith("Microsoft")) {
			return 1;
	...
	const usingsWithNamespace = sortedUsings.map((u) => ({
		using: u,
		namespace: getNamespace(u),
	}));
	const systemUsings = usingsWithNamespace
		.filter((u) => u.namespace.startsWith("System"))
		.map((u) => u.using);
	const microsoftUsings = usingsWithNamespace
		.filter(
			(u) =>
				!u.namespace.startsWith("System") &&
				u.namespace.startsWith("Microsoft"),
		)
		.map((u) => u.using);
	const otherUsings = usingsWithNamespace
		.filter(
			(u) =>
				!u.namespace.startsWith("System") &&
				!u.namespace.startsWith("Microsoft"),
	...
	const groups = [systemUsings, microsoftUsings, otherUsings].filter(
		(g) => g.length > 0,
	);
